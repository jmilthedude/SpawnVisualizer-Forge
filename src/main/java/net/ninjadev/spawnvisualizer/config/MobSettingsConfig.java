...
public class MobSettingsConfig extends Config {
    ...
    public List<ResourceLocation> getEntityIds() {
        return new ArrayList<>(mobEntries.keySet());
    }
    ...
    public static class MobConfig {
        ...
        public List<ResourceLocation> getWhitelistedBiomes(ResourceLocation worldId) {
            return this.getDimensionEntries().stream().filter(entry -> entry.getId().equals(worldId)).map(DimensionEntry::getBiomeWhitelist).flatMap(List::stream).collect(Collectors.toList());
        }

        public List<ResourceLocation> getBlacklistedBiomes(ResourceLocation worldId) {
            return this.getDimensionEntries().stream().filter(entry -> entry.getId().equals(worldId)).map(DimensionEntry::getBiomeBlacklist).flatMap(List::stream).collect(Collectors.toList());
        }

        public List<ResourceLocation> getValidDimensions() {
            return this.getDimensionEntries().stream().map(DimensionEntry::getId).collect(Collectors.toList());
        }
